...
CREATE TABLE pl1_final.races_final(
    year           TEXT, -- Proveniente de Temporada [season] (FK, PK)
    round          SMALLINT,
...
CREATE TABLE pl1_final.lap_times_final(
    ...
    position       SMALLINT,
    time           TEXT,
    year           TEXT, -- Proveniente de temporada, pero forma parte de la PK de Gran Premio (PK, FK)
...
CREATE TABLE pl1_final.results_final(
    ...
    CONSTRAINT results_fk3 FOREIGN KEY (gpRef, year) REFERENCES pl1_final.races_final(name, year) ON DELETE RESTRICT ON UPDATE CASCADE -- FKs que provienen de la PK compuesta por ambos atributos de Gran Premio
);
...
CREATE TABLE pl1_final.qualifying_final(
    ...
    q2                 TEXT,
    q3                 TEXT,
    year               TEXT, -- Proveniente de Gran Premio -> traspaso de la PK como FK
    CONSTRAINT qualifying_pk PRIMARY KEY (raceRef, driverRef, year),
    CONSTRAINT races_fk FOREIGN KEY (raceRef, year) REFERENCES pl1_final.races_final(name, year) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT drivers_fk FOREIGN KEY (driverRef) REFERENCES pl1_final.drivers_final(driverRef) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE pl1_final.pit_stops_final(
    ...
    year           TEXT, -- Proveniente de Temporada [season] (PK, FK)
    time           TEXT,
    duration       TEXT,
    CONSTRAINT pit_stops_pk PRIMARY KEY (lap, raceRef, driverRef, year),
    CONSTRAINT lap_times_fk FOREIGN KEY (lap, driverRef, raceRef, year) REFERENCES pl1_final.lap_times_final(lap, driverRef, raceRef, year) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
INSERT INTO pl1_final.circuits_final(circuitRef, name, location, country, lat, lng, alt, url)
    SELECT
        circuitRef, name, location, country, lat::REAL, lng::REAL, alt::INTEGER, url
        FROM
...
INSERT INTO pl1_final.drivers_final(driverRef, number, code, forename, surname, dob, nationality, url)
    SELECT
        driverRef, number::SMALLINT, code, forename, surname, dob, nationality, url
        FROM
...
INSERT INTO pl1_final.results_final(gpRef, pilotoRef, escuderiaRef, posicion, puntos, year)
    SELECT
        gp_temp.name, conductor_temp.driverRef, escu_temp.constructorRef, corre_temp.posicion::INTEGER, corre_temp.puntos::REAL, gp_temp.year -- Error de violacion de results_fk3
        FROM
...
INSERT INTO pl1_final.races_final(year, round, circuitRef, name, date, time, url)
    SELECT
        year::SMALLINT, round::SMALLINT, circuitRef, name, date::DATE, time::TIME WITHOUT TIME ZONE, url -- Error de ambiguedad
        FROM
...
INSERT INTO pl1_final.lap_times_final(raceRef, driverRef, lap, position, time, year)
    SELECT
        raceRef, driverRef, lap::SMALLINT, position::SMALLINT, time, year::SMALLINT -- Error en el select?
        FROM
            pl1_temp.lap_times_temp AS vueltas_temp JOIN pl1_temp.races_temp AS gp_temp ON vueltas_temp.raceId = gp_temp.raceId;
...
INSERT INTO pl1_final.qualifying_final(raceRef, driverRef, position, q1, q2, q3, year)
    SELECT
        raceRef, driverRef, position::SMALLINT, position::SMALLINT, q1::SMALLINT, q2::SMALLINT, q3::SMALLINT, year::SMALLINT
        FROM
...
INSERT INTO pl1_final.pit_stops_temp(raceRef, driverRef, lap, year, time, duration)
    SELECT
        raceRef, driverRef, lap::SMALLINT, year::SMALLINT, time, duration
        FROM
            pl1_temp.pit_stops_temp AS boxes_temp JOIN pl1_temp.races_temp AS gp_temp ON boxes_temp.raceId = gp_temp.raceId JOIN
            pl1_temp.drivers_temp AS conductor_temp ON boxes_temp.driverId = conductor_temp.driverId JOIN
            pl1_temp.lap_times_temp AS vueltas_temp ON boxes_temp.driverId = vueltas_temp.driverId AND boxes_temp.raceId = vueltas_temp.raceId;
